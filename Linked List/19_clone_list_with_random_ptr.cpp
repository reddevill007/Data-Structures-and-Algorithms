...
public:
    Node *copyList(Node *head)
    {
        ...
        {
            mapNode[orignal] = clone;
            orignal = orignal->next;
            clone = clone->next;
        }

        // step 3: arranging random pointer
        orignal = head;
        clone = cloneHead;

        // step 4: clone random pointer to right position
        while (orignal != NULL)
        {
            clone->arb = mapNode[orignal->arb];
            orignal = orignal->next;
            clone = clone->next;
        }
    ...
    Node *copyList2(Node *head)
    {
        ...
        // step 2: add cloneNode in between orignal list
        Node *orignal = head;
        Node *clone = cloneHead;

        while (orignal != NULL)
        {
            Node *next = orignal->next;
            orignal->next = clone;
            orignal = next;
        ...
        while (curr != NULL)
        {
            if (curr->next != NULL)
            {
                curr->next->arb = curr->arb ? curr->next->arb = curr->arb->next : curr->arb;
            }
            curr = curr->next->next;
        }
